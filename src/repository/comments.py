...
async def create_comment(
    db: Session, comment: schemas.CommentCreate, post_id: int, user_id: int
) -> models.Comment:
...
async def update_comment(
    db: Session, comment_id: int, comment: schemas.CommentUpdate
) -> models.Comment | None:
...
async def get_comments_breakdown(db: Session, date_from: datetime, date_to: datetime):
    stats = (
        ...
        .group_by(models.Comment.post_id, func.date(models.Comment.created_at))
        .all()
    )
    ...
    final_results = [
        {"post_id": post_id, "stats": stats} for post_id, stats in results.items()
    ]
