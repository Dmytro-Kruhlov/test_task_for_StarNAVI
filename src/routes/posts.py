...
async def create_post(
    ...
    cur_post = await repository_posts.create_post(
        db=db, post=post, user_id=current_user.id
    )
    if not cur_post:
        raise HTTPException(
...
async def delete_post(
    ...
    if post.user_id == current_user.id:
        ...
        raise HTTPException(
            ...
            detail="You cannot delete other users posts",
        )
...
async def update_post(
    ...
    if cur_post.user_id == current_user.id:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You cannot update other users posts",
        )
