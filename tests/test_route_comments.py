...
async def test_create_comment(client, session, token):
    ...
    response = client.post("api/comments/posts/1/comments/", json=comment_data, headers={"Authorization": f"Bearer {token}"})
...
async def test_create_comment_post_not_found(client, session, token):
    ...
    comment_data = {"content": "Test comment"}
    response = client.post("api/comments/posts/3/comments/", json=comment_data, headers={"Authorization": f"Bearer {token}"})
...
async def test_create_comment_post_blocked(client, session, token):
    comment_data = {"content": "Test comment"}
    response = client.post("api/comments/posts/2/comments/", json=comment_data, headers={"Authorization": f"Bearer {token}"})
...
async def test_read_comments(client, session, token):
    ...
    response = client.get("api/comments/posts/1/comments/", headers={"Authorization": f"Bearer {token}"})
...
async def test_read_nonexistent_comments(client, session, token):
    ...
    response = client.get("api/comments/posts/2/comments/", headers={"Authorization": f"Bearer {token}"})
...
async def test_get_comments_breakdown(client, session, token):
    ...
    assert data[0]["stats"][0]["total_comments"] == 2
    assert data[0]["stats"][0]["blocked_comments"] == 1
